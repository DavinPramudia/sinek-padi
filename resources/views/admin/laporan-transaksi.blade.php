<x-admin.layout-admin title="Laporan Transaksi">
    <div class="flex flex-col h-full space-y-6">
            ...
            <form method="GET" action="{{ route('admin.laporan-transaksi') }}" class="flex flex-wrap items-center gap-2 bg-[#141c1a] border border-[#243733] p-2 rounded-xl">
                ...
                <div class="flex items-center space-x-1 bg-[#0B0909] border border-[#243733] rounded-lg px-3 py-1.5">
                    ...
                    <select name="filter_type" id="filterType" onchange="changeFilterInput()" class="bg-transparent text-[#EDEDED] text-xs font-semibold focus:outline-none cursor-pointer">
                        ...
                        <option value="bulanan" {{ (request('filter_type', 'bulanan') == 'bulanan') ? 'selected' : '' }} class="bg-[#0B0909]">Bulanan</option>
                        <option value="tahunan" {{ (request('filter_type', 'tahunan') == 'tahunan') ? 'selected' : '' }} class="bg-[#0B0909]">Tahunan</option>
                        <option value="triwulanan" {{ (request('filter_type', 'triwulanan') == 'triwulanan') ? 'selected' : '' }} class="bg-[#0B0909]">Triwulanan</option>
                    </select>
                </div>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
            <form method="GET" action="{{ route('admin.laporan-transaksi') }}" class="relative w-full sm:w-72">
                <!-- Membawa parameter filter agar saat search filter tidak reset -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-[#141c1a] border border-[#243733] p-5 rounded-xl flex flex-col justify-center">
                <p class="text-sm text-[#d1d5dc] font-medium">Total Transaksi</p>
                <h3 class="text-2xl font-bold text-white mt-1">{{ $totalTransaksi }} Unit</h3>
            </div>
            <div class="bg-[#141c1a] border border-[#243733] p-5 rounded-xl flex flex-col justify-center">
                <p class="text-sm text-[#d1d5dc] font-medium">Total Pendapatan</p>
    ...
    <!-- Memanggil file JavaScript terpisah yang sudah dibuat di public/js/filter-laporan.js -->
    <script src="{{ asset('js/filter-laporan.js') }}"></script>
