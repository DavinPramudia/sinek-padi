        ...
        <form x-ref="saveForm" action="{{ route('admin.manajemen-akun.update', $user->id_users ?? $user->id) }}" method="POST" class="bg-[#141c1a] border border-[#243733] p-8 rounded-xl space-y-5 shadow-lg">
            ...
            <div x-data="{ showPassword: false }">
                <label class="block text-xs font-medium text-[#EDEDED] mb-2">
                    Password {{ isset($user) ? 'Baru' : '' }} 
                    @if(isset($user))
                        <span class="text-gray-500 font-normal">(Kosongkan jika tidak ingin mengubah password)</span>
                    @endif
                </label>
                <div class="relative">
                    <input :type="showPassword ? 'text' : 'password'" name="password" {{ isset($user) ? '' : 'required' }} class="bg-[#0B0909] border @error('password') border-red-500 @else border-[#243733] @enderror text-[#EDEDED] text-xs rounded-lg focus:ring-[#3aafa9] focus:border-[#3aafa9] block w-full p-3 pr-10 outline-none" placeholder="********">
            ...
            {{-- Ulangi Password Baru (Dengan Tombol Mata) --}}
            <div x-data="{ showConfirmPassword: false }">
                <label class="block text-xs font-medium text-[#EDEDED] mb-2">Ulangi Password {{ isset($user) ? 'Baru' : '' }}</label>
                <div class="relative">
                    <input :type="showConfirmPassword ? 'text' : 'password'" name="password_confirmation" {{ isset($user) ? '' : 'required' }} class="bg-[#0B0909] border border-[#243733] text-[#EDEDED] text-xs rounded-lg focus:ring-[#3aafa9] focus:border-[#3aafa9] block w-full p-3 pr-10 outline-none" placeholder="********">
