<div x-show="openSaveModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" style="display: none;" x-cloak>
    <div class="bg-[#141c1a] border border-[#243733] p-6 rounded-xl shadow-xl max-w-sm w-full text-center space-y-4">
        ...
        <div class="flex justify-center space-x-3 pt-2">
            ...
            {{-- Tombol Ya, Simpan (Memanggil activeForm yang sudah diset) --}}
            <button @click="if (window.activeForm) { window.activeForm.submit(); } else { $refs.saveForm.submit(); }" type="button" class="px-4 py-2 bg-[#C4C4FA] hover:bg-[#b0b0f7] text-[#0B0909] font-semibold text-xs rounded-lg transition cursor-pointer">
                Ya, Simpan
            </button>
        </div>
    </div>
